...
//int ByClientRemotePortKey = remote_endpoint.port();
//int sizeRecovByte = package.length();
//ePackage RPC = UNKNOW_PRMS;
...
void c_Logic::parsing_package(const std::string & package, boost::asio::ip::udp::endpoint & remote_endpoint)
{
	...
	byteArr.read((char*)&m_unBox_uÑlientPassport, 4);
	...
	default:
		std::cout << "cmd îò êëèåíòà íå ðàñïîçíàíà " << "\n";
		break;
	}
...
void c_Logic::eventSpawnVehicle(std::stringstream& byteArr, int sizeRecovBy, int byClientID)
{
	...
	if (
		m_DataBase->m_VehManager->regVehicle(want_car)
		)
	{
...
void c_Logic::refreshPlayerPossitions(std::stringstream& byteArr, int sizeRecovBy, int byClient)
{
	...
	m_DataBase->refreshPlayer(playerActor);
}
